...
#use lambda_handler as the function so to run it on lambda
def lambda_handler(event, context):
    
    #call out a sale in the vent variable dictionary
    sale = event["sale"]
    
    #next, process the sale with the process_transaction function
    result = process_transaction(sale)
    
    #if the result of process transaction is false, return bad code 400
    if result is False:
        return {"statusCode": 400, "body": "invalid transaction"}
    
    #if process_transaction is true, return the result
    else:
    
        #then we return the statuscode and the result after running the sale
